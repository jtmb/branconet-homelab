...
Same as above, but select only the role(s) you need. Example: add `longhorn` to an existing cluster.
...
Applications are managed via FluxCD. Each app lives in a Git repository under `02-applications/<name>/`.

### Adding a New App

1. Create a directory: `02-applications/my-app/`
...
4. Add the repo to Flux:
   ...
   curl -X POST http://localhost:4000/api/flux/repos \
     ...
     -d '{"repoUrl":"https://github.com/user/charts","branch":"main","path":"./02-applications/my-app"}'
   ```
5. Flux will sync within 30 seconds
...
curl -X POST http://localhost:4000/api/cluster/nodes \
  -H "Content-Type: application/json" \
  -d '{"name":"New Node","hostname":"u4","ipAddress":"192.168.1.13","role":"worker"}'
```
...
└── 02-applications/                 # FluxCD-managed apps
    ├── plex/
    ├── home-assistant/
    ├── discord-bots/
    └── ...
