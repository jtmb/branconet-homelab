...
cd ansible_provisioning && \
    ...
    --limit 'master_node' --skip-tags "reboot" --tags "images" \
    ...
    user_id=$userid  \
    discord_webook=$discord_webook PUB_IP=$public_ip cf_key=$cf_key cf_zone_id=$cf_zone_id domain_name=$domain_name domain_name=$domain_name email=$email \
